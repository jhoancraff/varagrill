...
function AnalystNewRecipePage({ isMobile, isAdmin, onBack }) {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        if (!response.ok || !data.ok) {
          throw new Error(data.message || 'No se pudieron cargar los datos para recetas.');
        }
        setIngredients(Array.isArray(data.ingredients) ? data.ingredients : []);
        setPreparations(Array.isArray(data.preparations) ? data.preparations : []);
      } catch (error) {
        setMessage(error.message || 'No se pudieron cargar los datos para recetas.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setMessage(data.message || 'Receta creada correctamente.');
      setForm(emptyForm);
      setDraft(emptyDraft);
      setComponents([]);
      setShowIngredientResults(false);
  ...
  if (!isAdmin) {
    return (
      <section style={containerStyle(isMobile)}>
        <div style={badgeStyle}>Nueva receta</div>
        <h2 style={titleStyle(isMobile)}>Acceso restringido</h2>
        <div style={noticeStyle}>Solo el rol Administrador puede crear recetas.</div>
  ...
  return (
    <section style={containerStyle(isMobile)}>
      ...
      <form onSubmit={handleSubmit} style={panelStyle}>
        {loading ? <div style={emptyStateStyle}>Cargando ingredientes y subrecetas...</div> : null}
            ...
            <div style={formGridStyle(isMobile)}>
              ...
              <label style={{ ...fieldStyle, gridColumn: isMobile ? 'auto' : '1 / -1' }}>
                ...
                <textarea
                  ...
                  rows={3}
                  placeholder="Describe preparación, porciones y notas operativas"
                />
              </label>
            </div>
...
export default AnalystNewRecipePage;
