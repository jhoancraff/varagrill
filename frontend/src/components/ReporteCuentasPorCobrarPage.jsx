...
function ReporteCuentasPorCobrarPage({ isMobile, onBack }) {
  ...
  const loadReport = useCallback(async (fechaConsultada) => {
    ...
    try {
      ...
      if (!response.ok || !json.ok) {
        throw new Error(json.message || 'No se pudo cargar las cuentas por cobrar.');
      }
      setData(json);
    } catch (error) {
      setMessage(error.message || 'No se pudo cargar las cuentas por cobrar.');
      setData(null);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <section style={containerStyle(isMobile)}>
      ...
      {loading ? <div style={emptyStyle}>Cargando cuentas por cobrar...</div> : null}
...
export default ReporteCuentasPorCobrarPage;
