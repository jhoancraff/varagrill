...
function ReporteVentasDiaPage({ isMobile, onBack }) {
  ...
  const loadReport = useCallback(async (fechaConsultada) => {
    ...
    try {
      ...
      if (!response.ok || !json.ok) {
        throw new Error(json.message || 'No se pudo cargar el detalle de ventas.');
      }
      setData(json);
    } catch (error) {
      setMessage(error.message || 'No se pudo cargar el detalle de ventas.');
      setData(null);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <section style={containerStyle(isMobile)}>
      ...
      {loading ? <div style={emptyStyle}>Cargando ventas...</div> : null}
...
export default ReporteVentasDiaPage;
