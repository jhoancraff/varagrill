...
function AnalystEditPreparationPage({ isMobile, preparationId, onBack }) {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        const selected = (data.recipes || []).find((item) => String(item.id) === String(preparationId));
        if (!selected) {
          throw new Error('La subreceta seleccionada no existe.');
        }
  ...
  const filteredPreparations = useMemo(() => {
    ...
    if (!query) {
      return preparations.filter((item) => String(item.id) !== String(form.id));
    }
    return preparations
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok || !data.ok) {
        throw new Error(data.message || 'No se pudo actualizar la subreceta.');
      }

      showSuccess(data.message || 'Subreceta actualizada correctamente.');
      markClean({ form, components });
    } catch (error) {
      showError(error.message || 'No se pudo actualizar la subreceta.');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <section style={containerStyle(isMobile)}>
      <h2 style={titleStyle(isMobile)}>Editar subreceta</h2>
            ...
            <div style={helperCardStyle}>
              ...
              <div style={composerStyle(isMobile)}>
                <div ref={ingredientPickerRef} style={pickerWrapStyle}>
                  <input value={draft.ingredientSearch} onChange={(e) => { setDraft((c) => ({ ...c, ingredientSearch: e.target.value, ingredientId: '' })); setShowIngredientResults(true); setShowPreparationResults(false); }} onFocus={() => { setShowIngredientResults(true); setShowPreparationResults(false); }} placeholder="Buscar ingrediente" style={inputStyle} />
                  {showIngredientResults ? <div style={pickerListStyle}>{filteredIngredients.map((item) => <button key={item.id} type="button" style={pickerItemStyle} onMouseDown={(e) => e.preventDefault()} onClick={() => { setDraft((c) => ({ ...c, ingredientId: String(item.id), ingredientSearch: item.nombre, preparationId: '', preparationSearch: '' })); setShowIngredientResults(false); }}>{item.nombre}</button>)}</div> : null}
                </div>
            ...
            <div style={helperCardStyle}>
              ...
              <div style={composerStyle(isMobile)}>
                <div ref={preparationPickerRef} style={pickerWrapStyle}>
                  <input value={draft.preparationSearch} onChange={(e) => { setDraft((c) => ({ ...c, preparationSearch: e.target.value, preparationId: '' })); setShowPreparationResults(true); setShowIngredientResults(false); }} onFocus={() => { setShowPreparationResults(true); setShowIngredientResults(false); }} placeholder="Buscar subreceta" style={inputStyle} />
                  {showPreparationResults ? <div style={pickerListStyle}>{filteredPreparations.map((item) => <button key={item.id} type="button" style={pickerItemStyle} onMouseDown={(e) => e.preventDefault()} onClick={() => { setDraft((c) => ({ ...c, preparationId: String(item.id), preparationSearch: item.nombre, ingredientId: '', ingredientSearch: '' })); setShowPreparationResults(false); }}>{item.nombre}</button>)}</div> : null}
                </div>
                <input type="number" min="0.001" step="0.001" value={draft.cantidad} onChange={(e) => setDraft((c) => ({ ...c, cantidad: e.target.value }))} placeholder="Cantidad" style={inputStyle} />
...
export default AnalystEditPreparationPage;