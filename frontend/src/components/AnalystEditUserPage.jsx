...
function AnalystEditUserPage({ isMobile, isAdmin, userId, onBack }) {
  ...
  useEffect(() => {
    ...
    const loadUser = async () => {
      ...
      try {
        ...
        const selectedUser = (data.users || []).find((entry) => String(entry.id) === String(userId));
        if (!selectedUser) {
  ...
  if (!isAdmin) {
    return (
      <section style={containerStyle(isMobile)}>
        <div style={badgeStyle}>Editar usuario</div>
        <h2 style={titleStyle(isMobile)}>Acceso restringido</h2>
        <div style={noticeStyle}>Solo el rol Administrador puede modificar usuarios.</div>
          ...
          <>
            <div style={formGridStyle(isMobile)}>
              ...
              <label style={fieldStyle}>
                ...
                <input type="password" value={form.password} onChange={(event) => handleChange('password', event.target.value)} placeholder="Déjala vacía para conservar la actual" style={inputStyle} />
              </label>
...
export default AnalystEditUserPage;