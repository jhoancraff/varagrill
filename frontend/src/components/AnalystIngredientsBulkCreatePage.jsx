...
function AnalystIngredientsBulkCreatePage({ isMobile, onBack }) {
  ...
  const handleConfirm = async () => {
    if (!motivo.trim()) {
      showError('Indica el motivo de esta carga (ej: "Inversión inicial - apertura de restaurante").');
      return;
    }
    ...
    if (items.length === 0) {
      showError('Marca al menos una fila para crear.');
      return;
    }
    ...
    try {
      ...
      setSummary(data);
      showSuccess('Carga completada.');
    } catch (error) {
      showError(error.message || 'No se pudo completar la carga.');
    } finally {
      setConfirming(false);
    }
  };
