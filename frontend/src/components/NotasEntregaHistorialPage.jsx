...
function NotasEntregaHistorialPage({ isMobile, onBack, embedded = false, refreshToken }) {
  ...
  const fetchNotas = useCallback(async (fechaBuscada) => {
    setLoading(true);
    try {
  ...
  return (
    <section style={containerStyle(isMobile, embedded)}>
      {!embedded ? (
        <div style={headerWrapStyle(isMobile)}>
          <div>
            ...
            <p style={subtitleStyle}>
              Busca una nota de entrega ya cobrada por fecha y reimprímela si el cliente necesita otra copia.
            </p>
          </div>
        ...
        <div style={embeddedHeaderStyle}>Historial de notas de entrega · Reimprimir</div>
      )}
      ...
      {!loading && !error && notas.length === 0 ? (
        <div style={emptyStateStyle}>No hay notas de entrega registradas en esa fecha.</div>
      ) : null}

      {!loading && !error && notas.length > 0 ? (
        <div style={listStyle}>
          {notas.map((nota) => (
            <div key={nota.id} style={notaRowStyle}>
              <div style={{ display: 'grid', gap: 2 }}>
                <div style={{ color: '#fff', fontWeight: 700 }}>Nota de entrega {nota.codigo}</div>
                <div style={{ color: '#d2c4c4', fontSize: 13 }}>
                  {nota.metodo_pago}
                  {' · '}
                  {new Date(nota.fecha_emision).toLocaleTimeString('es-VE', { hour: '2-digit', minute: '2-digit' })}
                  {' · '}
                  {nota.pedidos.length} pedido(s)
                </div>
                <div style={{ color: '#ffcf7d', fontWeight: 700 }}>
                  {formatMontoDocumento(nota.total, nota.moneda, nota.tasa_cambio_referencia)}
                </div>
              </div>
              <button
                type="button"
