...
function AnalystEditProductPage({ isMobile, isAdmin, productId, onBack, onProductsChanged }) {
    ...
    const loadProduct = async () => {
      ...
      try {
        ...
        const loadedForm = {
          ...
          margen_ganancia_pct: selectedProduct.margen_ganancia_pct || '',
        ...
        const loadedGruposOpciones = (Array.isArray(selectedProduct.grupos_opciones) ? selectedProduct.grupos_opciones : []).map((grupo) => ({
          uid: nextOpcionesUid('grupo'),
          tipo: grupo.categoria_opciones_id ? 'dinamico' : 'curado',
          nombre: grupo.nombre || '',
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      showError(error.message || 'No se pudo actualizar el producto.');
    } finally {
      setSaving(false);
    }
  };
